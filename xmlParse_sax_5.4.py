...
auditPath = ['E:\\myData\\Programmieren\\Python\\alogs']
Suffix = []
...
#PSW Profil Ausgabe in Datei "ergebnis.txt"
PSWPruefListe = ["rulename", "expirationcount", "minimumlength","maximumlength",
					"consecutivecharacters", "historycount", "casesensitive",
					"alphabetic", "numeric", "special"]
PSWProLi = PSWProfilListe
pswkeys = PSWProLi.keys()
for pswK, pswV in PSWProLi.items():
        pswprofs = pswV.keys()
for pswprof in pswprofs:
    d.write("Profile: "+pswprof+"\n")
    d.write("          ")
    for PL in PSWPruefListe:
        d.write("{:21}".format(PL[:21]))     #PL[:9] = nur die ersten 10 Zeichen des String werden angezeigt
    d.write("\n")
    for suf in pswkeys:
        d.write("HMC{:3}".format(suf)+"    ")
        for attrib in PSWPruefListe:
            try:
                d.write("{:21}".format(PSWProLi[suf][pswprof][attrib]))
            except (KeyError):
                d.write("{:21}".format("---"))
        d.write("\n")
    d.write("\n")
